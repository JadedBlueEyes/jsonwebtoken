...
fn round_trip_sign_verification_pem_pkcs1_jsonwebtoken_to_crate() {
    ...
    for (alg_rc, alg_ring) in RSA_ALGORITHMS {
        let encrypted =
            jsonwebtoken::crypto::sign("hello world".as_bytes(), &jsonwebtoken::EncodingKey::from_rsa_pem(privkey_pem).unwrap(), *alg_ring).unwrap();
        let is_valid =
            jsonwebtoken_rustcrypto::crypto::verify(&encrypted, "hello world", &jsonwebtoken_rustcrypto::DecodingKey::from_rsa(pubkey.clone()).unwrap(), *alg_rc)
                .unwrap();
...
fn round_trip_sign_verification_pem_pkcs1_crate_to_jsonwebtoken() {
    ...
    for (alg_rc, alg_ring) in RSA_ALGORITHMS {
        let encrypted =
            jsonwebtoken_rustcrypto::crypto::sign("hello world", &jsonwebtoken_rustcrypto::EncodingKey::from_rsa(privkey.clone()).unwrap(), *alg_rc).unwrap();
        let is_valid =
            jsonwebtoken::crypto::verify(&encrypted, "hello world".as_bytes(), &jsonwebtoken::DecodingKey::from_rsa_pem(pubkey_pem).unwrap(), *alg_ring)
                .unwrap();
        assert!(is_valid, "{:?} signature was not valid.", alg_rc);
    }
}